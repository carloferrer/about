  ...
  render() {
    ...
    const projectsItems = projects.map((project, parentIndex) => {
      ...
      return <li className="item" key={ `projectParent_${ parentIndex }` }>
        <h3 className="name"><a target="_blank" rel="noopener noreferrer" href={ `https://${ contact.github }/${ project.name }` }>{ project.name }<br/><span className="host">{ contact.github }/{ project.name }</span></a></h3>
